...
import {
  ...
  addDeviceRpc,
  subscribeRpc,
  leaseSubscriptionIpTx,
  leaseSubscriptionIpRpc,
  allocateIpTx,
  allocateIpRpc,
  initializeRootIpBlockTx,
  initializeRootIpBlockRpc,
  revokeIpTx,
  revokeIpRpc,
} from '../../sdk/utils'
...
const createSubscriber = async (
  ...
  await subscribeRpc({
    program,
    minDawnOut,
    deadline: new BN(currentTime.toString()).add(new BN(300)),
    caller: subscriberWallet.publicKey,
    signer: subscriberWallet,
...
export const leaseIpTests = () =>
  describe('dawn::lease_ip', () => {
    ...
    test('cannot lease IP with expired subscription', async () => {
      ...
      try {
        await leaseSubscriptionIpRpc({
          ...
          signer: subscriberSetup.wallet,
          devicePda: subscriberSetup.devicePda,
    ...
    test('cannot lease IP with device not owned by caller', async () => {
      ...
      try {
        await leaseSubscriptionIpRpc({
          ...
          devicePda: subscriberSetup.devicePda, // Device owned by subscriberSetup.wallet, not unauthorizedWallet
          ipRegistry: mock.subscriberIpRegistryPda,
          rootIpBlock: mock.rootSubscriberIpBlockPda,
          ipBlock: mock.ipBlockPda,
          ipLease: ipLeasePda,
    ...
    test('successfully leases first IP address from new block', async () => {
      ...
      const tx = await leaseSubscriptionIpTx({
        program,
        caller: subscriberSetup.wallet.publicKey,
        signer: subscriberSetup.wallet,
        devicePda: subscriberSetup.devicePda,
    ...
    test('handles multiple concurrent IP leases correctly', async () => {
      ...
      for (const subscriberSetup of subscribers) {
        ...
        const leasePromise = leaseSubscriptionIpRpc({
          program,
          caller: subscriberSetup.wallet.publicKey,
          signer: subscriberSetup.wallet,
          devicePda: subscriberSetup.devicePda,
  ...
  describe('dawn::allocate_ip', () => {
    ...
    test('cannot allocate IP for invalid tier', async () => {
      ...
      try {
        await allocateIpRpc({
          program,
          tier: 0, // Subscriber tier - should be rejected
          authority: wallet.payer.publicKey,
          signer: wallet.payer,
          devicePda,
          ipRegistry: mock.subscriberIpRegistryPda,
...
export const initializeRootIpBlockTests = () =>
  describe('dawn::initialize_root_ip_block', () => {
    test('cannot initialize root IP block without authority', async () => {
      ...
      try {
        await initializeRootIpBlockRpc({
          program,
          tier,
          baseIpv4: 0x0a400000,
          baseCidr: 14,
    ...
    test('successfully initializes root IP block for subscriber tier', async () => {
      ...
      const tx = await initializeRootIpBlockTx({
        program,
        tier,
        baseIpv4,
        baseCidr,
    ...
    test('cannot revoke IP lease with wrong caller', async () => {
      ...
      try {
        await revokeIpRpc({
          ...
          tier: 0,
          caller: unauthorizedWallet.publicKey,
          signer: unauthorizedWallet.payer,
          ipRegistry: mock.subscriberIpRegistryPda,
          rootIpBlock: mock.rootSubscriberIpBlockPda,
          ipBlock: mock.ipBlockPda,
          ipLease: ipLeasePda,
    ...
    test('cannot lease IP when subscription device mismatch', async () => {
      ...
      try {
        await leaseSubscriptionIpRpc({
          ...
          signer: subscriberSetup1.wallet,
          devicePda: subscriberSetup1.devicePda,
          ipRegistry: mock.subscriberIpRegistryPda,
          rootIpBlock: mock.rootSubscriberIpBlockPda,
          ipBlock: mock.ipBlockPda,
          ipLease: ipLeasePda,
